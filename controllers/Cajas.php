...
class Cajas extends Controller
{
    ...
    public function getDatos()
    {
        ...
        $data['egresos'] = number_format($compras + $gastos, 0, '.', '');
        $data['ingresos'] = number_format(($ventas + $apartados + $creditos) - $descuento, 0, '.', '');
        $data['montoInicial'] = (!empty($montoInicial['monto_inicial'])) ? number_format($montoInicial['monto_inicial'], 0, '.', '') : 0;
        $data['gastos'] = number_format($gastos, 0, '.', '');
        $data['egresossolo'] = number_format($compras, 0, '.', '');
        $data['saldo'] = number_format(($data['ingresos'] + $data['montoInicial']) -  $data['egresos'], 0, '.', '');

        $data['egresosDecimal'] = number_format($data['egresos'], 0);
        $data['ingresosDecimal'] = number_format($data['ingresos'], 0);
        $data['inicialDecimal'] = number_format($data['montoInicial'], 0);
        $data['gastosDecimal'] = number_format($data['gastos'], 0);
        // Agrege esta linea 
        $data['egresossoloDecimal'] = number_format($data['egresossolo'],0);

        $data['saldoDecimal'] = number_format($data['saldo'], 0);
    ...
    public function historialRepote($idCaja)
    {
        ...
        $data['movimientos']['saldoDecimal'] = number_format($datos['monto_final'] + $datos['monto_inicial'] - $datos['egresos'], 0);
        $this->views->getView('cajas', 'reporte', $data);
        $html = ob_get_clean();
        $dompdf = new Dompdf();
        $options = $dompdf->getOptions();
